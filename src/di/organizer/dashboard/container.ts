...
const bookingRepository = new BookingRepository(bookingEntityFactory);
 ...
 const getTicketsOverviewUseCase = new GetTicketsOverviewUseCase(organizerDashboardRepo);
 const getEarningsOverviewUseCase = new GetEarningsOverviewUseCase(organizerDashboardRepo);
 const getEventPerformanceUseCase = new GetEarningsPerformanceUseCase(organizerDashboardRepo);
 const payoutSummaryUseCase  = new GetPayoutSummaryUseCase(organizerDashboardRepo);
 const getSubscriptionSummaryUseCase  = new GetSubscriptionSummaryUseCase(organizerDashboardRepo);
 const getKycStatusUseCase = new GetKycStatusUseCase(organizerDashboardRepo);

 const organizerDashboardDetailsUseCase = new OrganizerDashboardDetailsUseCase(getTicketsOverviewUseCase, getEarningsOverviewUseCase,getEventPerformanceUseCase,payoutSummaryUseCase,getSubscriptionSummaryUseCase,getKycStatusUseCase)

export const organizerDashboardController = new OrganizerDashboardController(organizerDashboardUseCase, organizerDashboardDetailsUseCase);