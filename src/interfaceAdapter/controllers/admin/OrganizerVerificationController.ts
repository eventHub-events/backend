...
export class OrganizerVerificationController {
  ...
  async updateOrganizerUploadDocumentStatus(
    ...
    try {
      ...
      if (!organizerId || !data) {
        throw new CustomError(
          ErrorMessages.COMMON.ORGANIZER_ID_UPDATE_DETAILS_REQUIRED,
          HttpStatusCode.BAD_REQUEST
        );
      }
  ...
  async updateOverallVerificationStatus(
    ...
    try {
      ...
      const result =
        await this._organizerVerificationUseCase.updateOverallVerificationStatus(
          organizerId,
          dto
        );
      ...
      if (err instanceof ZodError) {
        throw new CustomError(
          ErrorMessages.COMMON.INVALID_INPUT,
          HttpStatusCode.BAD_REQUEST
        );
      }
