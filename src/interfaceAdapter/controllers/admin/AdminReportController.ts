...
export class AdminReportController {
 ...
 async fetchReports(req:  IAuthenticatedRequest, res: Response, next: NextFunction) : Promise<void> {
     try{
        const {targetType} = req.params;
        const {page, limit} =req.query;
        
        const data = await this._getReportUseCase.execute(targetType as ReportTypes , page as string, limit as string);
      res.status(HttpStatusCode.OK).json(ApiResponse.success(ResponseMessages.REPORT.REPORTS_FETCH_SUCCESS, HttpStatusCode.OK, data));
 ...
 async takeAction(req:  IAuthenticatedRequest, res: Response, next: NextFunction) : Promise<void> {
    try{

      const dto:AdminActionDTO = req.body;
      const result = await this._adminActionsUseCase.execute(dto);
    res.status(HttpStatusCode.OK).json(ApiResponse.success(ResponseMessages.REPORT.REPORT_UPDATE_SUCCESS, HttpStatusCode.OK, result));
    }catch(err){
        if(err instanceof NotFoundError) throw new CustomError(err.message,HttpStatusCode.NOT_FOUND);
       next(err)
    }
