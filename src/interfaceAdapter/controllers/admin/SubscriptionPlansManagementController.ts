...
export class SubscriptionPlansManagementController {
    ...
    try {
      ...
      if (!dto)
        throw new CustomError(
          ErrorMessages.SUBSCRIPTION.SUBSCRIPTION_DETAILS,
  ...
  async update(
    ...
    try {
      ...
      if (!dto)
        throw new CustomError(
          SubscriptionPlans.SUBSCRIPTION_PLANS_UPDATE_FAILURE,
          HttpStatusCode.BAD_REQUEST
        );

      const result = await this._updateSubscriptionPlanUseCase.execute(
        planId,
        dto
      );
      res
