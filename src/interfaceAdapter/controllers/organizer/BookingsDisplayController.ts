...
export class BookingsDisplayController {
  ...
  async fetchAllBookings(
    ...
    try {
      ...
      const { mappedBookings: bookings, totalPages } =
        await this._getAllBookingUseCase.execute({ organizerId, ...filters });
      res
  ...
  async fetchBookingDetailsById(
    ...
    try {
      ...
      if (!bookingId)
        throw new CustomError(
          ResponseMessages.BOOKING_DETAILS.BOOKING_DETAILS_FAILURE,
          HttpStatusCode.BAD_REQUEST
        );

      const bookingDetails =
        await this._getBookingDetailsById.execute(bookingId);
      res
        .status(HttpStatusCode.OK)
        .json(
          ApiResponse.success(
  ...
  async fetchBookingsByEventId(
    ...
    try {
      ...
      const { mappedBookings: bookings, totalPages } =
        await this._getBookingsByEventIdUseCase.execute({
          eventId,
          ...filters,
        });
      res
