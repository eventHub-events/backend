...
export class OrganizerDashboardController {
  ...
  async getDashboard(req: IAuthenticatedRequest, res :Response, next :NextFunction) : Promise<void> {
    try{
  ...
  async getDashboardDetails(req: IAuthenticatedRequest, res :Response, next :NextFunction) : Promise<void> {
    try{
         const organizerId = req.user!.id;
         console.log("query", req.query)
         const filter =  req.validatedQuery as OrganizerDashboardFilter;
         console.log("range is", filter)


     const data = await this._organizerDashboardDetailsUseCase.execute(organizerId, filter );
   res.status(HttpStatusCode.OK).json(ApiResponse.success(ResponseMessages.ORGANIZER_DASHBOARD.ON_SUCCESS,HttpStatusCode.OK, data));
    }catch(err){
      next(err)
    }
