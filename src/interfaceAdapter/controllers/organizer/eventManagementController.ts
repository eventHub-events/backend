...
export class EventManagementController {                                                                                                  
  ...
  async createEvent(req: IAuthenticatedRequest, res: Response, next: NextFunction): Promise<void> {
     try{
           ...
           if(!dto) throw new CustomError(ErrorMessages.EVENT.EVENT_DETAILS_REQUIRED, HttpStatusCode.BAD_REQUEST);

          const eventData =  await this._createEventUseCase.execute(dto);

    res.status(HttpStatusCode.CREATED).json(ApiResponse.success(ResponseMessages.EVENT.EVENT_CREATION_SUCCESS, HttpStatusCode.CREATED, eventData));
  ...
  async editEvent(req: Request, res: Response, next: NextFunction) :  Promise<void> {
   ...
   res.status(HttpStatusCode.OK).json(ApiResponse.success(ResponseMessages.EVENT.EVENT_UPDATE_SUCCESS, HttpStatusCode.OK, updatedEvent));
...
async Delete(req: Request, res: Response, next: NextFunction): Promise<void> {
  try{
    ...
    res.status(HttpStatusCode.OK).json(ApiResponse.success(ResponseMessages.EVENT.EVENT_DELETE_SUCCESS, HttpStatusCode.OK, result))
...
async cancel(req: IAuthenticatedRequest, res: Response, next: NextFunction): Promise<void> {
  try{
       const{ eventId} = req.params;
        ...
        res.status(HttpStatusCode.OK).json(ApiResponse.success(ResponseMessages.EVENT.EVENT_CANCEL_SUCCESS, HttpStatusCode.OK, result));
