  ...
  constructor(private _organizerProfileUseCase: IOrganizerProfileUseCase){}
    ...
    async  updateOrganizerProfile(req:Request, res:Response , next :NextFunction): Promise <Response|void>{
      ...
      if(!profileData) {
             ...
             throw new CustomError(ErrorMessages.PROFILE.DETAILS_EMPTY,HttpStatusCode.BAD_REQUEST);
      } 
        const updatedProfileData = await this._organizerProfileUseCase.updateOrganizerProfile(id,profileData);
    
         if(!updatedProfileData){

          throw new CustomError(ErrorMessages.PROFILE.UPDATE_FAILURE,HttpStatusCode.BAD_REQUEST);

        }
        return res.status(HttpStatusCode.CREATED).json(ApiResponse.success(ResponseMessages.PROFILE.PROFILE_CREATION_SUCCESS, HttpStatusCode.CREATED, updatedProfileData));
         ...
         try{
            const {id} = req.params ;
             ...
             if(!profileData){
              ...
              throw new CustomError(ErrorMessages.PROFILE.PROFILE_FETCH_FAILURE,HttpStatusCode.NOT_FOUND);

            }
            return res.status(HttpStatusCode.OK).json(ApiResponse.success(ResponseMessages.PROFILE.PROFILE_DATA_FETCH_SUCCESS, HttpStatusCode.OK, profileData));
