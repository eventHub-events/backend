...
export class ChatController {
  ...
  async startPrivateChat(
    ...
    try {
      ...
      const result = await this._startPrivateChatUseCase.execute(
        userId,
        organizerId,
        eventId
      );
  ...
  async getMessages(
    ...
    try {
      ...
      if (!conversationId)
        throw new CustomError(
          ErrorMessages.CONVERSATION.ID_REQUIRED,
          HttpStatusCode.BAD_REQUEST
        );
  ...
  async getOrganizerEventChats(
    ...
    try {
      ...
      if (!organizerId || !eventId)
        throw new CustomError(
          ErrorMessages.COMMON.EVENT_AND_ORGANIZER_ID_REQUIRED,
          HttpStatusCode.BAD_REQUEST
        );

      const chats = await this._getEventChatUseCase.execute(
        organizerId,
        eventId
      );
      res
  ...
  async getUserEventChats(
    ...
    try {
      ...
      if (!userId || !eventId)
        throw new CustomError(
          ErrorMessages.COMMON.USERID_AND_EVENTID,
          HttpStatusCode.BAD_REQUEST
        );

      const chats = await this._getEventChatForUserUseCase.execute(
        userId,
        eventId
      );
      res
