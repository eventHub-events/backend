...
export class ReviewController {
  ...
  async addEventReview(req: IAuthenticatedRequest, res: Response, next: NextFunction) : Promise<void> {
    try{
        const {eventId} = req.params;
        const dto: AddReviewDTO = req.body;
        const result = await this._addEventReviewUseCase.execute(eventId,dto);
    res.status(HttpStatusCode.CREATED).json(ApiResponse.success(ResponseMessages.REVIEW.REVIEW_CREATION_SUCCESS, HttpStatusCode.CREATED, result));

    }catch(err){
       if(err instanceof NotFoundError) throw new CustomError(err.message, HttpStatusCode.BAD_REQUEST);
       if(err instanceof UnauthorizedError) throw new CustomError(err.message, HttpStatusCode.BAD_REQUEST);
       next(err)
    }
   
  }
  async addOrganizerReview(req: IAuthenticatedRequest, res: Response, next: NextFunction) : Promise<void> {
    try{
        const dto: AddReviewDTO = req.body;
  ...
  async updateReview(req: IAuthenticatedRequest, res: Response, next: NextFunction) : Promise<void> {
    try{
