...
export class EventAnalyticsController {
 ...
 async getEventAnalytics(req : IAuthenticatedRequest, res :Response, next :NextFunction) : Promise<void> {
  try{
      const organizerId = req.user!.id;
      const filter = req.validatedQuery as EventAnalyticsFilter;

      const data = await this._getEventAnalyticsUseCase.execute(organizerId, filter);
    res.status(HttpStatusCode.OK).json(ApiResponse.success(ResponseMessages.EVENT_ANALYTICS.DETAILS_FETCH_SUCCESS,HttpStatusCode.OK,data));
  }catch(err){
