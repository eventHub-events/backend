...
export class ForgetPasswordUseCase implements IForgetPasswordUseCase{
  constructor(private _otpService:IGenerateOtpUseCase,private _userRepository:IUserRepository,private _loggerService:ILoggerService, private _emailService:IEmailService){}
   async forgetPassword( dto: ForgetPasswordDTO): Promise<{message:string}> {
     this._loggerService.info(`User with email ${dto.email} is going to resetPassword`)
      ...
      const otp= await this._otpService.executeForForgetPassword(dto.email,user)
      console.log("reset password otp",otp)
    await this._emailService.sendMail(  dto.email,
      'Your OTP code',
