...
export class GetUserChatEventUseCase implements IGetUserChatEventUseCase {
  ...
  async execute(
    ...
    const privateChats =
      await this._conversationRepo.findUserPrivateChatsByEvent(userId, eventId);
    if (privateChats.length === 0)
      throw new NotFoundError(ErrorMessages.CHAT.PRIVATE_CHATS_NOT_FOUND);

    const privateChatWithUnread = await Promise.all(
      privateChats.map(async chat => {
        const unreadCount = await this._messageRepo.countUnread(
          chat.id!,
          userId
        );
