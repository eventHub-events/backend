...
export class GetUserBookingListUseCase implements IGetUserBookingListUseCase {
  ...
  async execute(filter: BookingFilterDTO): Promise<{
    bookingsList: UserBookingListResponseDTO[];
    totalPages: number;
  }> {
    const { bookings, totalPages } =
      await this._bookingRepo.findAllWithFilter(filter);

    if (!bookings) {
      throw new NotFoundError(
        ErrorMessages.BOOKING.USER_BOOKING_LIST_NOT_FOUND
      );
    }

    const bookingsList =
      this._userBookingMapper.toUserResponseDTOList(bookings);
