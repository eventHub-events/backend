...
export  class GenerateTicketUseCase implements IGenerateTicketUseCase {
   ...
   async execute(booking: UserBookingListResponseDTO): Promise<string[]> {
      ...
      const url = await this._storageService.uploadBuffer(buffer, `tickets/${fileName}`);
      ...
      ticketUrls.push(url);
         ...
         const bookingEntity = await this._bookingRepo.findBookingById(booking.bookingId!);
