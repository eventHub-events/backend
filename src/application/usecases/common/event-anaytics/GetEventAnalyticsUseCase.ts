...
export class GetEventAnalyticsUseCase implements IGetEventAnalyticsUseCase {
 ...
 async execute(organizerId: string, filter: EventAnalyticsFilter): Promise<EventAnalyticsData> {
     if(!filter.eventId) new BadRequestError(ErrorMessages.EVENT.ID_REQUIRED);
     const result = await this._eventAnalyticsRepository.getEventAnalytics(organizerId, filter);

     if(!result) throw new NotFoundError(ErrorMessages.EVENT_ANALYTICS.DETAILS_NOT_FOUND);
     return result;
