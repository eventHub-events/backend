     ...
     if(report.targetType === ReportTypes.EVENT){
      ...
      // FOR REPORTING CHAT_MESSAGE //
             if(report.targetType === ReportTypes.CHAT_MESSAGE) {
                  ...
                  if(!targetPerson) throw new NotFoundError(ErrorMessages.REPORT.TARGET_PERSON_NOT_FOUND);

                  role = targetPerson.role === TargetPerson.USER? TargetPerson.USER: TargetPerson.ORGANIZER;
               ...
               await this._userRepo.updateUser(report.senderId!,{isBlocked: blocked});
      ...
      if(data.action === ReportActions.BLOCK && report.targetType !== ReportTypes.CHAT_MESSAGE){
