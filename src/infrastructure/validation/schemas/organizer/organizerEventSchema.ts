...
const locationSchema = z.object({
  venue: z
    ...
    .regex(
      venueRegex,
      'Venue must contain letters only and first letter should be capital'
    )
    .min(3, { message: 'Venue must be at least 3 characters' }),
  address: z.string().regex(addressRegex, {
    message:
      'Address must start with  a capital letter, contain only letters and be 5-50 characters long',
  }),
  city: z
    .string()
    .regex(cityRegex, {
      message:
        'City must contain letters only  and first letter  should be capital',
    })
  ...
  state: z
    ...
    .regex(cityRegex, {
      message:
        'State must contain letters only and first letter should be capital',
    })
  ...
  country: z
    ...
    .regex(
      cityRegex,
      'Country must contain letters only and first letter should be capital'
    )
    .min(3, { message: 'Country must be at least 3 characters' }),
  pincode: z
    .string()
    .regex(pinCodeRegex, { message: 'PinCode must contain exactly 6 digits' })
    .optional(),
...
export const organizerEventSchema = z.object({
  title: z
    ...
    .regex(titleRegex, {
      message:
        'Title must start  with a capital letter and contain only letters, numbers, and spaces.',
    }),
  ...
  category: z
    ...
    .regex(
      categoryRegex,
      'Category must contain letters only and first letter should be capital'
    )
