...
export class PdfReportService implements IPdfReportService {
  generateTransactionsPDF(
    ...
    // ---------- Draw header ----------
    doc.font("Helvetica-Bold").fontSize(30).text("EventHub - Transaction Report", {
      align: "center"
    });
    ...
    const drawTableHeader = () => {
      ...
      doc.moveTo(startX, cursorY).lineTo(startX + pageInnerWidth, cursorY).stroke();
      cursorY += 8;
    ...
    const drawFooter = () => {
      const footerY = doc.page.height - doc.page.margins.bottom - (footerReservedHeight - 20);
    ...
    const usableBottom = doc.page.height - doc.page.margins.bottom - footerReservedHeight;
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      if (cursorY + fixedRowHeight > usableBottom) {
        ...
        doc.font("Helvetica-Bold").fontSize(18).text("EventHub - Transaction Report (Continued)", {
          align: "center"
        });
        doc.moveDown(0.6);
        cursorY = doc.y + 6;
    ...
    if (cursorY + footerReservedHeight <= doc.page.height - doc.page.margins.bottom) {
      drawFooter();
    } else {
      doc.addPage();
      pageNumber++;
