...
export class JWTToken implements ITokenService {
  constructor(private _config: ITokenConfig) {}
  async generateToken(payload: object): Promise<string> {
    const token = jwt.sign(payload, this._config.jwtSecret, {
      expiresIn: this._config.accessTokenExpiry,
    });
  ...
  async generateResetToken(payload: object): Promise<string> {
    const token = jwt.sign(payload, this._config.jwtSecret, {
      expiresIn: this._config.resetTokenExpiry,
    });
    return token;
  }
  ...
  async generateRefreshToken(payload: object): Promise<string> {
    const token = jwt.sign(payload, this._config.jwtSecret, {
      expiresIn: this._config.refreshTokenExpiry,
    });
    return token;
  }
