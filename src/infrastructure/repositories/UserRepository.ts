...
export class UserRepository extends BaseRepository<IUserDocument>  implements  IUserRepository {
   ...
   async findAllWithFilter(filter : FilterQuery<UserEntity> = {}): Promise< UserEntity[] | null > {
   ...
   async updateUser (userId: string, data: Partial<UserEntity>): Promise<UserEntity> {
  ...
  async UpdateUserByEmail(email: string, data: Partial<UserEntity>) : Promise<UserEntity> {

         this._loggerService.info(`Updating user with email: ${email}`);
            const result = await super.findOneAndUpdate({ email }, data) as UserDbModel;
