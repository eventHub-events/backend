...
import { categoryModel, ICategoryDocument } from "../../db/models/admin/CategoryModel";
import { BaseRepository } from "../BaseRepository";

export class CategoryRepository extends BaseRepository<ICategoryDocument>  implements ICategoryRepository {

  constructor(
        private _categoryEntityFactory : ICategoryEntityFactory<categoryDbModel, CategoryEntity>
  ){
    super(categoryModel)
  }
...
async editCategory(categoryId: string, data: Partial<CategoryEntity>): Promise<CategoryEntity> {

     const updatedDoc = await super.update(categoryId, data) as categoryDbModel;
...
async fetchAllCategories(includeDeleted?: boolean ): Promise<CategoryEntity[]> {
      const filter = includeDeleted ? {} : {isDeleted: false};
    const categoryDocs = await super.findAll(filter) as categoryDbModel[];
    return categoryDocs.length ? this._categoryEntityFactory.toDomainList(categoryDocs) : [];
}
...
async softDeleteCategory(categoryId: string): Promise<void> {
     ...
     const deletedDoc = await super.update(categoryId, { isDeleted: true }) as categoryDbModel;
