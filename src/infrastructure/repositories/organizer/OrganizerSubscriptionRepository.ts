...
import {
  IOrganizerSubscriptionRepository,
  OrganizerSubscriptionAnalytics,
  PlanSubscriptionAnalytics,
  SubscriptionRevenueTimeline,
  SubscriptionRevenueTimelineItem,
  SubscriptionStatusSummary,
} from '../../../domain/repositories/organizer/IOrganizerSubscriptionRepository';
import { OrganizerSubscriptionDbModel } from '../../../domain/types/OrganizerTypes';
import {
  IOrganizerSubscription,
  OrganizerSubscriptionModel,
} from '../../db/models/organizer/subscription/OrganizerSubscriptionModel';
import { ReportRange } from '../../types/dashboard/booking';
import { BaseRepository } from '../BaseRepository';

export class OrganizerSubscriptionRepository
  extends BaseRepository<IOrganizerSubscription>
  implements IOrganizerSubscriptionRepository
{
  constructor(
  ...
  async fetchExpiredSubscription(
    ...
    const subscriptions = (await super.findAll(
      filter
    )) as OrganizerSubscriptionDbModel[];
  ...
  async getRevenueByRange(
    range: ReportRange
  ): Promise<SubscriptionRevenueTimeline> {
    const now = new Date();
