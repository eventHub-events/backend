...
export class  OrganizerProfileRepository extends BaseRepository<IOrganizerProfile> implements IOrganizerProfileRepository{
  constructor(
    ...
    private _organizerProfileEntityFactory: IOrganizerProfileEntityFactory< OrganizerProfileDbModel, OrganizerProfile, UserDbModel, OrganizerProfileWithUser>
    
  ){
    super(OrganizerProfileModel)
  }
  async createProfile(profileData: OrganizerProfile ): Promise<OrganizerProfile> {
   

       this._logger.info(`Creating Organizer profile for id:${profileData.organizerId }`)
      

      const profileDoc = await super.create( this._organizerProfileEntityFactory.toPersistence(profileData) ) as OrganizerProfileDbModel
      return this._organizerProfileEntityFactory.toDomain(profileDoc)
    
  }
 async getProfileWithUser(id: string): Promise< OrganizerProfileWithUser| null> {
      ...
      const doc = profileDoc as OrganizerProfileDbModel & {organizerId : UserDbModel}
      if(!profileDoc){
  ...
  async updateProfile(id: string, data: Partial< OrganizerProfile >): Promise< OrganizerProfile > {
      ...
      const updated = await super.findOneAndUpdate({organizerId:id},data) as OrganizerProfileDbModel ;
