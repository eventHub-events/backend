...
import { IEarningsOverview, IOrganizerEventPerformance, IOrganizerKycStatus, IOrganizerPayoutSummary, IOrganizerSubscriptionSummary, ITicketsOverview, OrganizerDashboardFilter } from "../../../domain/interface/organizer-dashboard/dashboard";
...
export class OrganizerDashboardRepository implements IOrganizerDashboardRepository {
 async getTicketsOverview(organizerId: string, filter?: OrganizerDashboardFilter) : Promise<ITicketsOverview> {
          ...
          tickets: [
            {
              $group: {
                ...
                cancelledTickets: {
                  $sum: {
                    $cond: [{ $eq: ["$status", "cancelled"] }, { $sum: "$tickets.quantity" }, 0]
                  }
                },
                refundedTickets: {
                  $sum: {
                    $cond: [{ $eq: ["$status", "refunded"] }, { $sum: "$tickets.quantity" }, 0]
                  }
                }
              }
            }
  ...
  async getEarningsOverview(organizerId: string, filter?: OrganizerDashboardFilter): Promise<IEarningsOverview> {
    const oid = new mongoose.Types.ObjectId(organizerId);
  ...
  to.setHours(23, 59, 59, 999);
    ...
    const agg = await BookingModel.aggregate([
      { $match: { organizerId: oid, status: { $in: ["confirmed", "refunded"] },
    ...
    const trend = await BookingModel.aggregate([
      { $match: { organizerId: oid, payoutStatus: "paid",
            createdAt: { $gte: from, $lte: to }
       } },
      {
        $group: {
  ...
  async getEventPerformance(
    organizerId: string,filter?: OrganizerDashboardFilter
  ...
  to.setHours(23, 59, 59, 999);
    ...
    const rows = await BookingModel.aggregate([
      { $match: { organizerId: oid, organizerAmount: { $gt: 0 },
