...
import {
  IOrganizerStripeAccount,
  OrganizerStripeAccountModel,
} from '../../db/models/organizer/organizer-stripe-account/OrganizerStripeAccountModel';
import { BaseRepository } from '../BaseRepository';

export class OrganizerStripeAccountRepository
  extends BaseRepository<IOrganizerStripeAccount>
  implements IOrganizerStripeAccountRepository
{
  constructor(
  ...
  async createStripeAccount(
    stripeAccountEntity: OrganizerStripeAccountEntity
  ): Promise<OrganizerStripeAccountEntity> {
    const organizerId = new Types.ObjectId(stripeAccountEntity.organizerId);
    const entityData = (await super.create({
      ...stripeAccountEntity,
      organizerId,
    })) as OrganizerStripeAccountDbModel;
  ...
  async getStripeAccountByStripeId(
    ...
    const stripeAccount = (await super.findOne({
      stripeAccountId,
    })) as OrganizerStripeAccountDbModel;
    return this._stripeAccountEntityFactory.toDomain(stripeAccount);
  }
  async getStripeAccountById(
    stripeAccountId: string
  ): Promise<OrganizerStripeAccountEntity> {
    const stripeAccount = (await super.findById(
      stripeAccountId
    )) as OrganizerStripeAccountDbModel;
    return this._stripeAccountEntityFactory.toDomain(stripeAccount);
  }
