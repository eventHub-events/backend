...
import {
  EventTicketingModel,
  IEventTicketing,
} from '../../db/models/organizer/events/EventTicketingModel';
import { BaseRepository } from '../BaseRepository';
import { ErrorMessages } from '../../../constants/errorMessages';

ErrorMessages;
export class EventTicketingRepository
  extends BaseRepository<IEventTicketing>
  implements IEventTicketingRepository
{
  constructor(
    private _eventTicketingEntityFactory: IEventTicketingEntityFactory<
  ...
  async createTicketing(
    data: EventTicketingEntity
  ): Promise<EventTicketingEntity> {
  ...
  async updateTicketingByEventId(
    eventId: string,
    data: EventTicketingEntity
  ): Promise<EventTicketingEntity> {
  ...
  async findTicketingById(ticketingId: string): Promise<EventTicketingEntity> {
    const ticketingDocs = (await super.findById(
      ticketingId
    )) as EventTicketingDbModel;

    if (!ticketingDocs)
      throw new Error(ErrorMessages.TICKETING.DETAILS_NOT_FOUND);
  ...
  async findTicketingByEventId(eventId: string): Promise<EventTicketingEntity> {
    const ticketingDocs = (await super.findOne({
      eventId,
    })) as EventTicketingDbModel;
    if (!ticketingDocs)
      throw new NotFoundError(ErrorMessages.TICKETING.DETAILS_NOT_FOUND);
