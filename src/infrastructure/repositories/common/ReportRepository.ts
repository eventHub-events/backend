...
import {
  IReport,
  ReportModel,
} from '../../db/models/common/Report/ReportModel';
...
{
  constructor(private _reportEntityFactory: IReportEntityFactory) {
    super(ReportModel);
  }
  ...
  async getReports(
    ...
    page: number,
    limit: number
  ): Promise<{ reportEntity: ReportEntity[]; totalPages: number }> {
    const filter = { targetType };
    const { data, total } = (await super.paginate(filter, page, limit)) as {
      data: ReportDbModel[];
      total: number;
    };
