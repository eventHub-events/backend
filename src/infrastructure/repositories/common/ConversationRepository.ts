...
import {
  ...
  ConversationType,
  IConversation,
} from '../../db/models/common/chat/ConversationModel';
...
{
  constructor(private _conversationEntityFactory: IConversationEntityFactory) {
  ...
  async findUserPrivateChatsByEvent(
    userId: string,
    eventId: string
  ): Promise<ConversationEntity[]> {
    const filter = {
  ...
  async findOrCreateCommunityConversation(
    eventId: string
  ): Promise<ConversationEntity> {
  ...
  async getById(conversationId: string): Promise<ConversationEntity> {
    const conversation = (await super.findById(
      conversationId
    )) as ConversationDbModel;
    return this._conversationEntityFactory.toDomain(conversation);
  }
  ...
  async updateLastMessage(
    ...
    if (!updated)
      throw new NotFoundError(
        ErrorMessages.CHAT.CONVERSATION_NOT_FOUND_FOR_UPDATE
      );
  }
  async isParticipant(
    conversationId: string,
