...
import {
  ...
  OrganizerEventPerformanceResult,
...
{
  constructor(
    private _bookingEntityFactory: IBookingEntityFactory<
      BookingDbModel,
      BookingEntity
    >
  ) {
  ...
  async findAllWithFilter(
    ...
    if (filter.organizerName)
      cleanFilter.organizerName = {
        $regex: filter.organizerName,
        $options: 'i',
      };
    if (filter.userName)
...
if (dateRange) {
  cleanFilter.createdAt = dateRange;
    ...
    const bookings = this._bookingEntityFactory.toDomainList(
      data as BookingDbModel[]
    );
  ...
  async fetchBookingByPaymentIntentId(
    paymentIntentId: string
  ): Promise<BookingEntity> {
    const booking = (await super.findOne({
      paymentIntentId,
    })) as BookingDbModel;
    return this._bookingEntityFactory.toDomain(booking);
  }

  async findBookingsByEventIdAndUserId(
    eventId: string,
    userId: string
  ...
  async findBookingsByOrganizerIdAndUserId(
    organizerId: string,
    userId: string
  ): Promise<BookingEntity | null> {
  ...
  async getRevenueAndBookingSByRange(
    range: ReportRange
  ): Promise<RevenueAndBookingSummary> {
    const now = new Date();
  ...
  async getOrganizerRevenueByRange(
    organizerId: string,
    range: ReportRange
  ): Promise<OrganizerRevenueTimeline> {
    const now = new Date();
    const from = new Date();
  ...
  async getOrganizerEventPerformance(
    organizerId: string
  ): Promise<OrganizerEventPerformance[]> {
  ...
  async getOrganizerPayoutSummary(
    organizerId: string
    ...
    const [doc] = await BookingModel.aggregate<{
      ...
      },
      {
        $group: {
          ...
          pendingCount: {
            $sum: {
              $cond: [{ $eq: ['$payoutStatus', PayoutStatus.PENDING] }, 1, 0],
            },
          },
  ...
  async getOrganizerEventPerformanceForTable(
    organizerId: string,
    page: number,
    limit: number
  ): Promise<OrganizerEventPerformanceResult> {
