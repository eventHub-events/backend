...
export class AdminSocketService extends BaseSocketService {
  ...
  constructor(
    ...
    private _userManagementUseCase: IFetchUserUseCase
  ) {
  ...
  private async handleBlockUser(
    socket: Socket,
    { targetUserId, isBlocked }: { targetUserId: string; isBlocked: boolean }
  ) {
    try {
      const result = await this._userManagementUseCase.updateUser(
        targetUserId,
        { isBlocked }
      );

      console.log('sockets', [...this._userNamespace.sockets.values()]);
      const targetSocket = [...this._userNamespace.sockets.values()].find(
