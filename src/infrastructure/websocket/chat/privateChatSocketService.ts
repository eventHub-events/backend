...
export class PrivateChatSocketService {
  ...
  private onConnection(socket: Socket) {
    ...
    socket.on(
      'chat_open',
      async ({ userId, conversationId }: ChatOpenState) => {
    ...
    socket.on(
      'join_private_room',
      ({ conversationId, peerId, userId }: JoinPrivateRoomPayload) => {
    ...
    socket.on('send_private_message', async (data: PrivateMessagePayload) => {
      ...
      this.nameSpace
        .to(conversationId)
        .emit('private_message_received', { ...data, isRead });

      const receiverSocketId = this.onlineUsers.get(receiverId);

      if (receiverSocketId) {
        console.log(
          '🔔 Sending direct alert to:',
          receiverId,
          receiverSocketId
        );
