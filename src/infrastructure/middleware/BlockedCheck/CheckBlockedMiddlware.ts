...
export  class CheckBlockedMiddleware {
 ...
 execute = async(req :IAuthenticatedRequest, res: Response, next :NextFunction) => {
    ...
    const user = await this._userRepo.findUserById(userId);
       ...
       if(user.isBlocked){
        console.log("🔴 USER IS BLOCKED — RETURNING 403");
          return res.status(HttpStatusCode.FORBIDDEN).json(ApiResponse.error(ErrorMessages.AUTH.BLOCK_ERROR, HttpStatusCode.FORBIDDEN,ErrorMessages.ERROR_CODES.USER_BLOCKED,user.role));
       }
